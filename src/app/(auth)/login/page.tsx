...
import { Phone, ShieldCheck, ArrowRight, ArrowLeft, Sparkles, MessageSquareText, Star, MonitorSmartphone, LogOut } from 'lucide-react';
...
import { useAuth, type OtherDeviceSession, type TryWebLoginResult } from '@/hooks/useAuth';
...
const LABEL_CLS = 'block text-[12.5px] font-semibold text-ink-800 dark:text-slate-300 mb-1.5';
...
export default function LoginPage() {
  ...
  const [otherDevice, setOtherDevice] = useState<OtherDeviceSession | null>(null);
  ...
  useEffect(() => {
    ...
    if (new URLSearchParams(window.location.search).get('reason') === 'session-replaced') {
  ...
  const applyLoginOutcome = (outcome: TryWebLoginResult, mobileNumber: string): boolean => {
    ...
    if ('alreadyLoggedIn' in outcome) {
      setPendingMobile(mobileNumber);
      setOtherDevice(outcome.alreadyLoggedIn);
      setStep('conflict');
      return true;
    }
  ...
  const handleAdminQuickLogin = async () => {
    ...
    try {
      ...
      if ('alreadyLoggedIn' in outcome) return;
      if (!outcome.devOtp) {
  ...
  const handleResend = async () => {
    ...
    try {
      if (isAdminChallenge) {
        ...
        if (typeof outcome === 'object' && 'otpRequired' in outcome) {
          setDevOtp(outcome.devOtp);
          setOtp('');
          setResendIn(RESEND_COOLDOWN_SECONDS);
        }
  ...
  const handleForceLogin = async () => {
    clearError();
    try {
      await tryWebLogin(pendingMobile, true);
    } catch {
      /* `error` from useAuth already carries the message */
    }
  };
    ...
    >
      ...
      <div className="relative z-10 min-h-0 flex-1 overflow-y-auto px-5 py-4">
        <div className="flex min-h-full flex-col items-center justify-center gap-5">
          ...
          {/* ═══════════ Floating card ═══════════ */}
          <div className="w-full max-w-[400px] rounded-[28px] p-[1px] bg-gradient-to-b from-white/20 via-white/5 to-transparent shadow-[0_40px_90px_-24px_rgba(0,0,0,0.65)]">
            <div className="relative rounded-[27px] bg-white dark:bg-slate-900 p-5 sm:p-6 overflow-hidden">
              ...
              <div className="relative mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 shadow-[0_16px_32px_-12px_rgba(37,99,235,0.55)]">
                <Phone className="h-5 w-5 text-white" strokeWidth={2} />
              </div>

              <div className="relative">
                {step === 'conflict' && otherDevice ? (
                  <ConflictStep
                    device={otherDevice}
                    isLoading={isLoading}
                ...
                <p className="mt-4 flex items-start justify-center gap-1.5 text-[11.5px] text-ink-500">
...
function MobileStep({
  ...
  return (
    <>
      ...
      <h2 className="display-section text-[1.5rem] sm:text-[1.75rem] dark:text-white mb-2">
        Sign in to Exam Ready
      </h2>
      <p className="lede text-[13.5px] dark:text-slate-400 mb-5">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            <div className="pointer-events-none absolute left-0 top-0 bottom-0 flex items-center gap-2 pl-3.5 pr-3 border-r border-slate-200 dark:border-slate-700">
              <span className="text-[13.5px] font-bold text-ink-800 dark:text-slate-200">+91</span>
            </div>
            <input
              id="mobileNumber"
              type="tel"
              ...
              autoFocus
              className={cn(
                'peer h-[46px] w-full rounded-xl border bg-white pl-[68px] pr-4 text-[15px] font-semibold tracking-wide text-ink-900',
          ...
          {errors.mobileNumber ? (
            ...
            <p className="mt-1.5 text-[11.5px] text-ink-500 dark:text-slate-500">
        ...
        <Button
          ...
          className="btn-premium group h-[46px] w-full rounded-xl border-none text-[14.5px] font-bold"
        >
          Continue
        ...
        {onAdminQuickLogin && (
          <>
            ...
            <button
              ...
              disabled={isLoading}
              className="flex h-[42px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-orange-300/70 bg-orange-50 text-[13.5px] font-bold text-[#b9450d] transition-all hover:-translate-y-px hover:border-orange-400 hover:bg-orange-100 hover:shadow-md disabled:opacity-60 dark:border-orange-400/30 dark:bg-orange-500/10 dark:text-orange-300"
            >
...
function ConflictStep({
  ...
  device: OtherDeviceSession;
  isLoading: boolean;
  onContinue: () => void;
  onCancel: () => void;
}) {
  const lastActive = device.lastActiveAt
    ? new Date(device.lastActiveAt).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })
    : null;

  return (
    <>
      <span className="eyebrow-line mb-3 text-[#c95817]">Account in use</span>
      <h2 className="display-section mb-3 text-[1.625rem] sm:text-[1.875rem] dark:text-white">
        You&apos;re already signed in elsewhere
      </h2>
      <p className="lede mb-6 text-[14.5px] dark:text-slate-400">
        This account can be used on one device at a time. Sign out the other device to continue here.
      </p>

      <div className="mb-6 flex items-center gap-3 rounded-2xl border border-orange-200/70 bg-orange-50/70 p-4 dark:border-orange-400/25 dark:bg-orange-500/10">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#f4953f] via-[#e2691f] to-[#c4501a] text-white shadow-md shadow-orange-600/30">
          <MonitorSmartphone className="h-5 w-5" />
        </span>
        <div className="min-w-0">
          <p className="truncate text-[14.5px] font-bold text-ink-900 dark:text-white">{device.device}</p>
          {lastActive && <p className="text-[12.5px] text-ink-600 dark:text-slate-400">Last active {lastActive}</p>}
        </div>
      </div>
      ...
      <button
        ...
        onClick={onCancel}
        disabled={isLoading}
        className="mt-3 h-11 w-full cursor-pointer rounded-xl text-[14px] font-semibold text-ink-600 transition-colors hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
      >
        Cancel
      </button>
    </>
